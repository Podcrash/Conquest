...
public class SeismicSlam extends Instant {
    ...
    public SeismicSlam(Player player, int level) {
        ...
        this.reach = (float) FastMath.pow((5.5d + this.level * 0.5d), 2d);
        this.damage = 1 + this.level;
        this.hitGround = new CollideBeforeHitGround(player)
                .changeEvaluation(() -> System.currentTimeMillis() - getLastUsed() >= 200L && ((Entity) getPlayer()).isOnGround())
                .then(() -> {
                    if (!usage) return;
                    usage = false;
